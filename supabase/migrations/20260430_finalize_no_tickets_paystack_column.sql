-- Issuing tickets must not require tickets.paystack_reference (legacy DBs may not have it).
-- Idempotency: payment_receipts.reference (unique). Reference is not stored on each ticket row.
...
begin
  ...
  if exists(select 1 from public.payment_receipts where reference = p_reference) then
