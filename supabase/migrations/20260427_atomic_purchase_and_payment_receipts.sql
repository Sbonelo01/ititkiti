...
begin
  ...
  if v_is_new_format then
    for v_idx in 0..jsonb_array_length(p_ticket_selections) - 1 loop
      v_ticket_type_id := (p_ticket_selections->v_idx->>'ticketTypeId')::uuid;
      v_selection_quantity := coalesce((p_ticket_selections->v_idx->>'quantity')::integer, 0);
      ...
      for v_idx in 1..v_selection_quantity loop
    ...
    for v_idx in 1..p_quantity loop
      insert into public.tickets (
        event_id,
        attendee_name,
        email,
